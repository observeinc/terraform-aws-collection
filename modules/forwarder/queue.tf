locals {
  ...
  sqs_arn    = "arn:${data.aws_partition.current.partition}:sqs:${local.region}:${local.account_id}:${var.name}"
    ...
    Statement = [
      {
        ...
        Condition = {
          ArnEquals = {
            "aws:SourceArn" = [for name in var.source_bucket_names : "arn:${data.aws_partition.current.partition}:s3:::${name}"]
          }
        }
      },
      {
